...
1. `advertised_product`
   - 本广告实际推广的商品。
   - 必须生成独立商品参考图，允许进入视频模型参考。
2. `story_prop`
   - 推动剧情或被人物交互的普通物品。
   - 在分镜板中解决，不进入视频模型参考。
3. `set_dressing`
...
2. `text_prompt_control`
   ...
   - 不生成独立道具图，在分镜板画面中固定；视频提示词只描述其已有动作，不把它作为参考素材。

3. `generate_independent_prop`
   - 广告商品必须使用；剧情关键物品仅在分镜板一致性确有必要时使用。
   - 生成独立资产不等于允许进入视频参考，视频参考仍只允许 `advertised_product`。
...
这些字段供下游 `generate-asset-prompts` 和 `generate-storyboard-prompts` 使用：
- 关键物品的提示词强调外观一致性锁定
  ...
  "characters": [
    {
      ...
      "reference_layout": "character_turnaround_21x9_v1",
  ...
  "props": [
    {
      ...
      "notes": "普通手机不生成独立视频参考，由分镜板固定"
    },
    {
      "asset_type": "prop",
      "asset_name": "广告商品",
      "seedance_label": "广告商品",
      "business_role": "advertised_product",
      "generation_required": true,
      "handling_policy": "generate_independent_prop",
      "is_key_item": true,
      "recurrence_count": 3,
      "appearances": ["S001", "S005", "S012"],
      "output_prompt_path": "./outputs/assets/props/prompts/广告商品.md",
      "notes": "广告商品必须生成独立参考并允许进入视频模型"
    }
  ]
}
```
